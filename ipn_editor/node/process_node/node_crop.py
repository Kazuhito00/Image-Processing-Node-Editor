...
class Node(DpgNodeABC):
    ...
    def update(
        ...
        # 領域指定
        min_x = float(dpg.get_value(input_value02_tag))
        max_x = float(dpg.get_value(input_value03_tag))
        min_y = float(dpg.get_value(input_value04_tag))
        max_y = float(dpg.get_value(input_value05_tag))
        if min_x > max_x:
            min_x, max_x = max_x - 0.01, min_x + 0.01
            dpg.set_value(input_value02_tag, min_x)
            dpg.set_value(input_value03_tag, max_x)
        if min_y > max_y:
            min_y, max_y = max_y - 0.01, min_y + 0.01
            dpg.set_value(input_value04_tag, min_y)
            dpg.set_value(input_value05_tag, max_y)
